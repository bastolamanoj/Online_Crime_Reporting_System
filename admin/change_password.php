...
<div class="container1">
    ...
    <div class="main-section1 ">
        ...
        <header>
            <h2> <i class="fa-solid fa-gauge" style="margin-right:13px;"></i>Change Password </h2>
        </header>
        ...
        <?php
            if(isset($_POST['submit'])){
                ...
                if($old_pass!="" && $new_pass!="" && $cnew_pass!=""){
                    $sql = "SELECT * FROM admin WHERE password = '$old_pass'";
                    $fire_sql = mysqli_query($conn, $sql);
                    $row = mysqli_num_rows($fire_sql);
                    
                    if($row){
                        if($new_pass == $cnew_pass){
                            $updatequery = "UPDATE admin SET password = '$new_pass'";
                            $updateresult = mysqli_query($conn, $updatequery);
                            if($updateresult){
                                session_destroy();
                                echo '<meta http-equiv="refresh" content="0; URL=index.php?msg=passchange"/>';
                            }
                        } else{
                            echo '
